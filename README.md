...
### 🖥️ PM2 관리 시스템
- **GUI 관리 도구**: `monitor.bat` 배치 파일로 쉬운 관리
- **백그라운드 실행**: PM2를 통한 안정적인 서비스 운영
...
### 일반적인 문제
1. **프로세스가 시작되지 않음**: `monitor.bat` → `f` (프로세스 이름 수정)
2. **포트 충돌**: `monitor.bat` → `p` (다른 포트 사용)
3. **로그가 안 보임**: `monitor.bat` → `3` → `1` (모든 로그)
4. **CMD 창이 계속 뜸**: PM2 서비스 설치 또는 앱 재시작

### 고급 문제 해결
1. **PM2 데몬 문제**: `monitor.bat` → `c` (안전 정리)
2. **메모리 누수**: 메모리 사용량 확인 후 재시작
3. **로그 파일 손상**: `pm2 flush` 후 재시작
